...
import PresetsModal from '@/features/settings/components/PresetsModal.vue';
...
function handleOpenPresets(): void {
  showSettings.value = false;
...
<template>
  <div class="arena-container min-h-screen flex relative overflow-hidden"
      ...
      <div class="absolute top-4 right-4 z-30 flex flex-col items-end gap-2 sm:gap-3"
        ...
        <div class="flex gap-2 sm:gap-3">
          ...
          <VTooltip placement="bottom" :delay="{ show: 600, hide: 0 }">
            <a href="https://github.com/WarLikeLaux/strike-v" target="_blank" rel="noopener noreferrer"
          ...
          <VTooltip placement="bottom" :delay="{ show: 600, hide: 0 }">
            ...
            <template #popper>
              <div class="text-sm">Настройки</div>
            </template>
          </VTooltip>
        </div>
      ...
      <div class="absolute top-6 left-0 right-0 z-20 flex items-center justify-between px-4">
        ...
        <div v-if="isViewingSideQuest" class="hidden sm:flex flex-wrap sm:flex-nowrap items-center gap-2 max-w-full">
          <VTooltip placement="bottom" :delay="{ show: 600, hide: 0 }">
            ...
            <template #popper>
              <div class="text-sm">Вернуться к основной скале</div>
            </template>
          </VTooltip>
    ...
    <Transition name="fade">
      <SettingsPanel v-if="showSettings" @close="showSettings = false" @open-help="handleOpenHelp"
        @open-presets="handleOpenPresets" />
    </Transition>
    ...
    <Transition name="fade">
      <HelpPanel v-if="showHelp" @close="showHelp = false" @back="showHelp = false; showSettings = true" />
    </Transition>

    <Transition name="fade">
      <PresetsModal v-if="showPresets" @close="showPresets = false" @back="showPresets = false; showSettings = true" />
    </Transition>
  </div>
</template>
